const firstSelectCurrency = document.getElementById(
    'currency-one'
) as HTMLSelectElement;
const SecondSelectCurrency = document.getElementById(
    'currency-two'
) as HTMLSelectElement;
const firstAmountNumber = document.getElementById(
    'amount-one'
) as HTMLInputElement;
const secondAmountNumber = document.getElementById(
    'amount-two'
) as HTMLInputElement;
const swapBtn = document.getElementById('swap') as HTMLButtonElement;
const rateStatus = document.getElementById('rate') as HTMLDivElement;
...
const currencyCaculateAPI = () => {
    fetch('https://open.exchangerate-api.com/v6/latest')
        ...
        .then((data) => {
            const rate = (
                data.rates[SecondSelectCurrency.value] /
                data.rates[firstSelectCurrency.value]
            ).toFixed(2);

            secondAmountNumber.value = (
                Number(firstAmountNumber.value) * Number(rate)
            ).toFixed(2);
